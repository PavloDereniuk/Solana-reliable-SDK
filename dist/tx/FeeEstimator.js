...
export class FeeEstimator {
     ...
     */
    async estimate(writableAccounts = [], level = 'medium') {
        ...
        try {
            ...
            const fees = await conn.getRecentPrioritizationFees(pubkeys.length ? { lockedWritableAccounts: pubkeys } : undefined);
            this.pool.reportSuccess(conn);
            if (fees.length === 0)
    ...
    async heliusFee(endpoint, accountKeys, level) {
        const url = new URL(endpoint);
        const apiKey = url.searchParams.get('api-key');
        if (!apiKey)
            throw new Error('no api-key in Helius endpoint');
        ...
        if (!resp.ok)
            throw new Error(`Helius ${resp.status}`);
        const data = (await resp.json());
        const fee = data.result?.priorityFeeEstimate;
        if (fee === undefined)
            throw new Error('unexpected Helius response shape');
        return fee;
    }
}
