...
export class MetricsCollector {
    ...
    // ── Transaction metrics ──────────────────────────────────────────────────────
    recordTransaction(sample) {
        this.txSamples.push({ ...sample, timestamp: Date.now() });
        if (this.txSamples.length > 1_000)
            this.txSamples.shift();
    }
    // ── Snapshots ────────────────────────────────────────────────────────────────
    getSnapshot() {
        const rpc = {};
...
// ── helpers ───────────────────────────────────────────────────────────────────
function gauge(name, value, attrs, timeUnixNano) {
    return {
        name,
        gauge: {
            dataPoints: [
